...
{
/// Write-only track handle. `update()` re-runs the snapshot through the writer
/// (idempotent on `file_path` — rekordcrate dedups, so re-adding is a no-op).
/// All getters/setters throw `unsupported_operation`: the adapter never reads
/// back a Rekordbox export.
class track_impl : public djinterop::track_impl
{
public:
    track_impl(std::shared_ptr<rekordcrate_writer> writer, int64_t id) :
        djinterop::track_impl{id}, writer_{std::move(writer)}
    {
    }
    ...
    // ---- All accessors/setters unsupported (write-only) ----
    ...
    std::vector<crate> containing_crates() override;
    ...
    void set_last_played_at(
        std::optional<std::chrono::system_clock::time_point> time) override;
